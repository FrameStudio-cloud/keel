...
export default function NewOrderModal({ onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div
        ref={trapRef}
        className="bg-surface-1 rounded-2xl border border-border-subtle p-6 w-full max-w-lg mx-4 max-h-[85vh] overflow-y-auto"
        role="dialog"
        aria-modal="true"
        aria-label="New order"
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-medium text-text-primary">New order</h2>
        ...
        <div className="flex flex-col gap-4">
          <div>
            ...
            {suggestions.length > 0 && (
              <div className="mt-1 border border-border-subtle rounded-lg overflow-hidden bg-surface-1">
                {suggestions.map((c) => (
                  <button
                    key={c.id}
                    type="button"
                    onClick={() => pickCustomer(c)}
                    className="w-full flex items-center gap-2 px-3 py-2 text-xs text-left text-text-body hover:bg-surface-2 transition-all"
                  >
                    <FiUser size={12} className="text-text-faint" />
                    {c.name} ({c.phone})
                  </button>
                ))}
              </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs text-text-faint">Items</label>
              <button type="button" onClick={addRow} className="flex items-center gap-1 text-xs text-brand hover:text-brand-strong transition-all"><FiPlus size={12} /> Add line</button>
            </div>
                    ...
                    <div key={i} className="bg-surface-2 rounded-xl p-3 space-y-2">
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          className="flex-1 border border-border-subtle rounded-lg px-3 py-2.5 sm:py-2 text-sm bg-surface-1 text-text-primary focus:outline-none focus:border-brand"
                        >
                        ...
                        {svc && svc.pricing_mode !== "flat" && (
                          <div className="w-14 sm:w-16">
                            <input
                              ...
                              className="w-full border border-border-subtle rounded-lg px-2 sm:px-3 py-2.5 sm:py-2 text-sm bg-surface-1 text-text-primary focus:outline-none focus:border-brand text-center"
                              title={svc.pricing_mode === "per_weight" ? "Weight in kg" : "Quantity"}
                            />
                          </div>
                        )}
                        {rows.length > 1 && (
                          <button type="button" onClick={() => removeRow(i)} className="p-2 sm:p-1.5 text-danger hover:text-danger transition-all"><FiTrash2 size={15} /></button>
                        )}
                      </div>
                      ...
                      />
                      {svc && (
                        <p className="text-xs text-text-faint text-right">{formatPrice(lineTotal)}</p>
                      )}
                    </div>
                  );
                })
          ...
          <div>
            <label className="text-xs text-text-faint mb-1 block">Payment method</label>
            <select
              value={paymentMethod}
          ...
          <div>
            <label className="text-xs text-text-faint mb-1 block">Order notes</label>
            <input
              value={orderNotes}
          ...
          <div>
            <label className="text-xs text-text-faint mb-1 block">Schedule (optional)</label>
            <input
              type="datetime-local"
              value={scheduledAt}
          ...
          <div className="bg-brand-muted rounded-lg px-4 py-3 flex justify-between items-center">
            ...
            <span className="text-sm font-medium text-brand">{formatPrice(total)}</span>
          </div>
        </div>
