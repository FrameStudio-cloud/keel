      ...
      <SectionCard icon={FiCamera} title="Store Branding">
        <div className="flex items-start gap-4">
          <button
            ...
            className="group relative w-16 h-16 shrink-0 rounded-2xl overflow-hidden border-2 border-dashed border-border-subtle hover:border-brand dark:hover:border-brand transition-all"
          >
            {logoUrl ? (
          ...
          <div className="min-w-0 pt-1">
            <h2 className="text-lg font-bold text-text-primary truncate">{storeName || "Untitled Store"}</h2>
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              <span className="text-[10px] font-semibold px-2.5 py-0.5 rounded-full bg-brand-muted text-brand border border-brand-soft dark:border-blue-500/20">
...
function InfoRow({ icon: Icon, label, value, link }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-surface-2 dark:bg-white/[0.03]">
      ...
      <span className="text-xs text-text-faint w-16 shrink-0">{label}</span>
      {link ? (
        <a href={value} target="_blank" rel="noopener noreferrer" className="text-sm text-brand truncate hover:underline ml-auto">
          {value.replace(/^https?:\/\//, "")}
        </a>
      ) : (
        <span className="text-sm text-text-primary truncate ml-auto">{value}</span>
      )}
    </div>
  );
}
