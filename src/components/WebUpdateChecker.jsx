...
export default function WebUpdateChecker() {
  ...
  const knownBuildRef = useRef(null);
  ...
  useEffect(() => {
    async function check() {
      try {
        ...
        const current = data.buildTime;

        if (knownBuildRef.current === null) {
          knownBuildRef.current = current;
        } else if (current !== knownBuildRef.current) {
          knownBuildRef.current = current;
          setShowBar(true);
          setDismissed(false);
        }
