...
export default function Marketing() {
  ...
  function printCatalog() {
    ...
    printWin.document.write(`
      ...
      <table><thead><tr><th>Product</th><th>Category</th><th>Price</th></tr></thead><tbody>
      ${products.map((p) => `<tr><td>${htmlEscape(p.name)}</td><td>${htmlEscape(p.category)}</td><td class="price">${formatPrice(p.price)}</td></tr>`).join("")}
      </tbody></table>
  ...
  useEffect(() => {
    (async () => {
      ...
      const text = qrType === "website" ? (websiteUrl || window.location.origin) : (raw ? `https://wa.me/${raw}` : "");
      setQrUrl(text);
  ...
  async function toggleNewArrival(item, value) {
    ...
    setProducts((prev) => prev.map((c) => (c.id === item.id ? { ...c, new_arrival: value } : c)));
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-[#16213e] rounded-xl border border-gray-100 dark:border-white/10 p-5">
          ...
          <div className="flex flex-col sm:flex-row gap-2 mb-3">
            ...
            >
              ...
              {products.map((c) => (<option key={c.id} value={c.id}>{c.name} — {formatPrice(c.price)}</option>))}
            </select>
            <button
        ...
        <div className="bg-white dark:bg-[#16213e] rounded-xl border border-gray-100 dark:border-white/10 p-5">
          ...
          <p className="text-xs text-gray-400 dark:text-slate-500 mb-3">
            {products.length} product(s) in your inventory{products.length === 100 ? " (showing first 100)" : ""}. Generate a print-friendly version.
          </p>
          <button
