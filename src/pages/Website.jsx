...
export default function Website() {
  ...
  if (!hasWebsite) {
    return (
      <PageLayout title="Website">
        <Helmet><title>Website — Keel</title></Helmet>
        <div className="relative overflow-hidden bg-surface-1 rounded-xl border border-border-subtle p-8 sm:p-12 text-center" style={{ backgroundImage: MESH }}>
          <div className="relative z-10">
            <div className="w-16 h-16 mx-auto mb-5 rounded-2xl bg-brand-muted flex items-center justify-center ring-4 ring-brand-soft/50 dark:ring-brand/10">
              <FiGlobe className="text-brand" size={30} />
            </div>
            <h2 className="text-xl font-semibold text-text-primary mb-2">
              You haven't set up your website yet
            </h2>
            <p className="text-sm text-text-muted mb-10 max-w-md mx-auto leading-relaxed">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-2xl mx-auto mb-10">
              <div className="bg-white/80 dark:bg-white/[0.04] backdrop-blur-sm rounded-xl border border-border-subtle dark:border-white/[0.06] p-4 text-center group hover:border-blue-200/50 dark:hover:border-blue-500/20 transition-all">
                <div className="flex flex-col items-center gap-2.5 mb-2.5">
                  ...
                  <div className="flex gap-1.5 w-full justify-center">
                    <span className="text-[9px] font-medium text-chart-4">Hero</span>
                    <span className="text-[9px] text-text-faint">·</span>
                    <span className="text-[9px] font-medium text-chart-3">Sale</span>
                    <span className="text-[9px] text-text-faint">·</span>
                    <span className="text-[9px] font-medium text-brand">Info</span>
                  </div>
                </div>
                <p className="text-xs font-semibold text-text-body">Banners</p>
              </div>

              <div className="bg-white/80 dark:bg-white/[0.04] backdrop-blur-sm rounded-xl border border-border-subtle dark:border-white/[0.06] p-4 text-center group hover:border-blue-200/50 dark:hover:border-blue-500/20 transition-all">
                <div className="flex flex-col items-center gap-2 mb-2.5">
                  <div className="w-full bg-surface-2 dark:bg-white/[0.06] rounded-lg px-2.5 py-2">
                    ...
                    <div className="flex gap-0.5 justify-center">
                      {["mon","tue","wed","thu","fri","sat","sun"].map((d,i) => (
                        <div key={d} className={`w-2 h-2 rounded-full ${i < 6 ? "bg-blue-400" : "bg-surface-2"}`} />
                      ))}
                    </div>
                  </div>
                </div>
                <p className="text-xs font-semibold text-text-body">Business Info</p>
              </div>
              ...
              <div className="bg-white/80 dark:bg-white/[0.04] backdrop-blur-sm rounded-xl border border-border-subtle dark:border-white/[0.06] p-4 text-center group hover:border-blue-200/50 dark:hover:border-blue-500/20 transition-all">
                ...
                <p className="text-xs font-semibold text-text-body">Gallery</p>
              </div>

              <div className="bg-white/80 dark:bg-white/[0.04] backdrop-blur-sm rounded-xl border border-border-subtle dark:border-white/[0.06] p-4 text-center group hover:border-blue-200/50 dark:hover:border-blue-500/20 transition-all">
                <div className="flex flex-col items-center gap-2 mb-2.5 relative">
                  <div className="bg-brand-muted rounded-xl px-2.5 py-1.5 w-full">
                    <div className="h-2 w-16 rounded-full bg-surface-2 mx-auto mb-1" />
                    <div className="h-2 w-12 rounded-full bg-surface-2 mx-auto" />
                  </div>
                  <div className="w-7 h-7 rounded-full bg-brand-soft flex items-center justify-center shadow-lg shadow-brand/30 absolute -bottom-1 -right-1">
                    <div className="w-3 h-3 rounded-full bg-surface-1" />
                  </div>
                </div>
                <p className="text-xs font-semibold text-text-body">Chat Widget</p>
              </div>
            </div>

            <Link
              to="/settings"
              className="inline-flex items-center gap-2 px-6 py-2.5 bg-brand hover:bg-brand-soft text-white text-sm font-semibold rounded-lg transition-all shadow-lg shadow-brand/25"
            >
              Go to Settings
  ...
  return (
    <PageLayout title="Website">
      ...
      <div className="flex gap-1 mb-6 border-b border-border-subtle overflow-x-auto overflow-hidden">
        {TABS.map((tab) => (
          <button
            key={tab.id}
