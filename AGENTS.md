...
- **Semantic theme tokens + codemod (Aug 2026)**: All dashboard colors migrated from raw Tailwind palette classes (gray/slate/blue/red/green-*) to **semantic tokens** — CSS variables defined in the `@theme` block of `src/index.css`. Roles (one job per token, never mixed): `surface-0..3` (page→card→chip/inset→raised), `border-subtle/strong`, `text-primary/body/muted/faint` (hierarchy), `brand-*` (+`brand`/`brand-strong`/`brand-soft`/`brand-muted`/`brand-contrast`), `accent-*` (warmth/highlights), `success/warning/danger/info` (status ONLY), `chart-1..6` (categorical data-viz, hue-separated). Tokens are **fully dark-adaptive**: the `[data-theme="keel-dark"]` + `[data-theme="midnight"]` blocks recolor the whole app at runtime with zero re-renders, so a translucent `dark:` half after a token twin (`bg-brand-muted dark:bg-blue-500/15`, `border-danger dark:border-red-500/20`, `text-success dark:text-success`) is **redundant and removed**. 4 themes in `src/lib/themes.js` (keel-light, forest, keel-dark, midnight) applied via `data-theme` attr + `keel-theme` localStorage (inline no-flash script in `index.html`), per-shop `theme_id` via SettingsProvider (migration `20260802_backfill_theme_ids.sql`), picked in Settings → Preferences `ThemePicker.jsx`. Solid-green buttons use `text-success-contrast` (dark-adaptive text, dark-mode value `#06281e` on light-green `#34d399`) — **never** `text-white`. Migration tooling in `scripts/`: `migrate-theme-tokens.mjs` (idempotent single-pass longest-match codemod; `--check` = dry-run) + `check-colors.mjs` (guardrail, exit 1 on any raw palette class in migrated files; intentional art — gradients, social platform rails, banner swatches, WhatsApp dots, storefront previews — allowlisted). Excluded (stay raw, not migrated/checked): `src/components/home/`, blog, Features/UseCases/AboutFramestudio/Terms/Privacy/PublicProduct/Login, `WhatsAppMockup.jsx`, and 5 storefront art components. Legacy theme stored `"light"/"dark"` maps to keel-light/keel-dark.
- **Blog redesigned as "The Keel Journal" magazine (Aug 2026)**: `/blog` and `/blog/:slug` rebuilt from utilitarian card grids into a classic editorial magazine. Index (`src/pages/Blog.jsx`) has a newspaper masthead (double-rule header, "Keel · Nairobi, Kenya", today's date + Issue number), a section/date rule row with an underline-style search input, a category rail (tag filters), a full-width **Cover story** lead (featured post = newest match, left gradient cover + right headline/excerpt/byline), and an asymmetric editorial grid (`lg:grid-cols-12` with varied spans 7/5/4/4/4/6/6 + staggered `sm:translate-y-6` offset on odd cells) showing "This week in the journal". Post pages (`src/pages/BlogPost.jsx`) have a fixed blue reading-progress bar, a bordered article header (kicker, serif headline, italic serif standfirst/excerpt, byline/date/read-time), a gradient cover band, body with blue-rule headings + drop-cap first paragraph (`first-letter:` on first block), "Filed under:" tag chips, and a "More from the journal" strip (same-tag posts first). Since `blog.json` `image` fields are all `null`, covers are **typographic gradient posters** (`src/components/blog/PostCover.jsx`) derived from a 6-palette rotation in `src/lib/blog.js` — no stock photos/broken images. Editorial serif: **Newsreader Variable** self-hosted via `@fontsource-variable/newsreader` (imported in `main.jsx`, `--font-serif` theme token in `index.css`). Scroll reveals via GSAP ScrollTrigger (already a dep, same pattern as Homepage) gated behind `prefers-reduced-motion`. Search + tag filter are client-side over `src/data/blog.json`. Lint/build/tests all green.
- **Integration app-store detail pages (Aug 2026)**: `/integrations/:slug` pages rebuilt from a single config card into full app-store detail pages — gradient hero band (icon, name, tagline, category chip, status pill + Set up/Upgrade to Pro/Connected CTA), "Why connect this" benefit grid, "How it works" numbered steps (WhatsApp shows a pure-Tailwind phone chat mockup alongside), stepper-guided setup wizard, and a FAQ accordion. Registry entries (`src/lib/integrations.js`) now carry `category`, `benefits`, `steps`, `faq`, `goals` so every integration gets a full page for free. Setup wizard starts with a goals checklist ("What do you want to use it for?", Continue disabled until ≥1) saved to the new generic **`integration_goals`** table (`shop_id, integration_slug, goals jsonb`, UNIQUE(shop_id, integration_slug), RLS tenant-scoped) — collects client-intent data for roadmap/pitch. Connected integrations show usage stats (WhatsApp: conversations handled/answered from `chat_messages`; Calendar: service orders synced via `service_orders.calendar_event_id`) plus a "next step" link. Pro lock now handled by the detail page (hero CTA + upsell panel), not inside cards. Migration `20260801_add_integration_goals.sql` applied.
...
Homepage (`src/pages/Homepage.jsx`) is a thin orchestrator (~60 lines) — Helmet + FAQPage JSON-LD + section composition. All sections live in `src/components/home/`: HomeNav, HomeHero, HomeFeatures, HomeHowItWorks, HomeWebsite, HomeTestimonials, HomeFaq, HomeCta (CTA + folded-in Contact at `#contact`), HomePwa, HomePricing, HomeFooter. Data (`howItWorks`, `testimonials`, `faqs`, `socialLinks`, `navGroups`) moved to `src/data/home.js`. Each section self-contains its own scoped `useGSAP` reveal gated behind `prefers-reduced-motion` (no reduced-motion → no animation). Hero shows a single real Keel screenshot (`dashboard-laptop-preview.webp`, replaced the 4 tilted AI mockups). Features section breaks the old 5-identical-zigzag into 2 alternating rows + a 3-card grid using the `Modern …` / `Payment summary dashboard widget` marketing screenshots from `public/` (via PictureImg → webp). How It Works is a static numbered 3-step strip (replaced the auto-cycling flashcard GSAP timeline). Website Integration mobile marquee is now a **pure CSS `home-marquee` keyframe** (translateX -50%, 30s linear, reduced-motion off) in `index.css` — replaced the JS `requestAnimationFrame` + `scrollLeft` loop. Stale "Download for Windows .msi" desktop section (Tauri was removed) replaced by HomePwa — "Works on any device / installable PWA" gradient band.
...
Dark mode uses `@variant dark` in Tailwind v4, scoped to `[data-theme="keel-dark"]` and `[data-theme="midnight"]`. Colors are **semantic tokens** (CSS variables in `@theme` — `bg-surface-1`, `text-text-muted`, `border-border-subtle`, `bg-brand-muted`, ...); raw palette classes in dashboard files fail `scripts/check-colors.mjs`. Tokens are dark-adaptive, so most `dark:` variants are no longer needed — only non-token utilities (e.g. translucent `bg-white/5` glass effects) still use them.
**Default theme is `"keel-light"`** — changed from `"dark"` to prevent flash on reload (previously, default was `"dark"` but `<html>` had no dark class, causing a light→dark flash after settings loaded).
Settings are fetched once by `SettingsProvider` context and consumed via `useSettings()` hook.
Theme is applied via `data-theme` attribute in SettingsProvider (`theme_id` → `src/lib/themes.js`), no flash since default is `keel-light`.
...
Tailwind v4 — no `tailwind.config.js`, dark mode via `@variant dark (&:where([data-theme="keel-dark"], [data-theme="keel-dark"] *, [data-theme="midnight"], [data-theme="midnight"] *));`.
...
node scripts/check-colors.mjs              # token guardrail — exit 1 on raw palette classes in migrated files
node scripts/migrate-theme-tokens.mjs --check   # codemod dry-run (should report 0 changes)
```
...
- `integration_goals` — id, shop_id (FK), integration_slug, goals (jsonb array), updated_at; UNIQUE(shop_id, integration_slug); RLS tenant-scoped via `auth.uid()` → `users` → shop
...
- `src/lib/integrations.js` — **app-store registry** (`INTEGRATIONS`, `getIntegration()`, `getIntegrationStatus()`, `COMING_SOON`). Each entry defines `slug`, `name`, `tagline`, `category`, `icon`, `tileClass` (gradient), `tier` (feature key from tiers.js or null = free), `component` (setup/manage view), `getStatus`, plus page content: `benefits[]` (icon/title/desc), `steps[]` (how-it-works), `faq[]` (q/a), `goals[]` (setup checklist), optional `featured: true` (store-page banner) and `preview` (marketing mockup component). Adding a new integration = one registry entry + one component — grid/routing/detail-page/status all derive from the registry automatically. `COMING_SOON` teasers (name/tagline/category/icon) render as muted store tiles.
...
- `src/hooks/useIntegrationGoals.js` — reads/saves setup goals to `integration_goals` (`select` + upsert on `shop_id,integration_slug` conflict)
- `src/pages/Integrations.jsx` — **integration store grid**: gradient hero band (search pill, trust chips incl. live connected count), category filter chips (derived from registry), wide featured-integration banner card (from `featured: true`), tile grid per integration (status pill + category chip), and a "Coming soon" section rendering `COMING_SOON` teasers + dashed "More coming soon" tile. Replaced the old `/bots` page (route `/bots` now redirects here).
- `src/pages/IntegrationDetail.jsx` — full app-store detail page for `/integrations/:slug`: back button ("Connect your tools") → gradient hero band (icon, name, tagline, category chip, status pill + Set up / Upgrade to Pro / Connected CTA, `scrollToSetup` anchor) → "Why connect this" benefit grid → "How it works" (renders `integration.preview` mockup side-by-side when present) → setup/manage section (`#setup`, Pro lock renders upsell panel instead of the component) → FAQ accordion. Unknown slug redirects to grid. Deep-linkable (detail components self-fetch).
- `src/components/integrations/WhatsAppBotCard.jsx` — WhatsApp setup/manage wizard: Stepper (Goal → Connect → Verify → Live), goals checklist first (saved to `integration_goals`, required ≥1), connect number → 6-digit code (resume-on-reload via `code_sent`), live panel with toggle + usage stats ("conversations handled / answered automatically" from `chat_messages`) + next-step link to `/website`. Pro gating handled by the page.
- `src/components/integrations/GoogleCalendarCard.jsx` — Google Calendar setup/manage: goals checklist → OAuth connect (preserves `?calendar=connected` handling), connected panel with "N service orders synced" (`service_orders.calendar_event_id` count), disconnect. Not-configured state when no `VITE_GOOGLE_CLIENT_ID`.
- `src/components/integrations/IntegrationHero.jsx` — gradient hero band for detail pages (icon, name, tagline, category, status CTA)
- `src/components/integrations/BenefitCards.jsx` — "Why connect this" benefit grid from registry `benefits`
- `src/components/integrations/HowItWorks.jsx` — numbered explainer from registry `steps`
- `src/components/integrations/FaqSection.jsx` — accordion from registry `faq`
- `src/components/integrations/GoalsStep.jsx` — reusable setup checklist ("What do you want to use it for?"), Continue disabled until ≥1 selected
- `src/components/integrations/WhatsAppMockup.jsx` — pure-Tailwind WhatsApp phone chat mockup (customer ↔ bot bubbles) shown on the WhatsApp detail page
- `src/components/integrations/IntegrationStats.jsx` — connected-integration stat chips (icon/value/label)
...
- `src/pages/Homepage.jsx` — thin orchestrator (~60 lines): Helmet + FAQPage JSON-LD + composition of `src/components/home/` sections (HomeNav, HomeHero, HomeFeatures, HomeHowItWorks, HomeWebsite, HomeTestimonials, HomeFaq, HomeCta, HomePwa, HomePricing, HomeFooter). Data in `src/data/home.js`.
- `src/pages/Blog.jsx` — "The Keel Journal" magazine index: newspaper masthead (double-rule header + date/issue row), underline-style search, category rail (client-side tag filter), Cover story lead, asymmetric editorial grid (`lg:grid-cols-12`, spans 7/5/4/4/4/6/6, odd cells offset `sm:translate-y-6`), GSAP ScrollTrigger reveals (gated on `prefers-reduced-motion`)
- `src/pages/BlogPost.jsx` — article page: fixed reading-progress bar, bordered header (kicker/serif H1/italic serif standfirst/byline), gradient cover band, blue-rule serif headings, drop-cap first paragraph, "Filed under:" tag chips, "More from the journal" strip (same-tag posts first), related via `postIndex` for PostCover palette
- `src/components/blog/PostCover.jsx` — typographic gradient "issue cover" poster (kicker, serif title, index number, "Read the story") — replaces `blog.json` `image` (all null); size `sm`/`md`/`lg`, aspect set by parent
- `src/lib/blog.js` — blog helpers: `formatBlogDate`, `readingTime`, `kicker(tags)`, `coverStyle(index)` (6-palette gradient rotation)
...
| `/blog` | Blog.jsx | "The Keel Journal" magazine index — masthead, category rail, Cover story lead, asymmetric editorial grid |
| `/blog/:slug` | BlogPost.jsx | Magazine article page — reading progress, serif standfirst, drop cap, "More from the journal" |
...
- **Paystack subscription renewal**: `src/components/RenewModal.jsx` — 3-step modal (plan select → pay → success), `window.PaystackPop.setup()` with `callback: function(){}` (not async arrow — Paystack's validation chokes on async). Edge function at `supabase/functions/verify-paystack-subscription/` (version 4, `verify_jwt: false`). **Must handle OPTIONS preflight** (CORS) on the edge function — browsers send OPTIONS before POST to cross-origin edge functions. CORS headers (`access-control-allow-origin: *`) on every response. Now uses **live** Paystack keys (set by user). Plan amounts: 50000 kobo (Basic, extends 30d), 100000 kobo (Pro, extends 30d + sets `chat_config.plan_tier="pro"`). Tested end-to-end working.

## Pricing
- **Free trial**: 7 days (was 30). New signups get `subscription_expires_at = now() + 7 days`.
- **Basic**: KSh 500/month — sales, inventory, service orders, expenses, customer management, stock alerts, basic reports.
- **Pro**: KSh 1,000/month — everything in Basic plus live website & storefront, P&L charts & CSV/PDF export, social media scheduler, WhatsApp & Telegram bots, M-Pesa reconciliation, QR codes & print catalogue, website analytics, data export.
- Renewals extend subscription by 30 days per payment.
- **Admin lock**: `shops.locked_at` — when set, LockoutScreen shows "Account locked — Contact support" with no renewal option. Cleared by admin unlock or on renewal.
...
- **Toolchain system**: Each template has a `toolchain` field in both dashboard (`storefrontBlueprints.js`) and provisioner (`registry.js`). The provisioner `provision.js` reads `config.toolchain.ui` from the deploy payload, resolves a template variant directory (e.g. `classic-heroui`), and falls back to the base template dir if the variant doesn't exist. This enables stacked UI library support — currently: `cite_ui` (classic inline) and `heroui` (HeroUI-powered variant with lucide-react icons + Inter/Outfit fonts).
- **`classic-heroui` template** (`src/templates/classic-heroui/`): 6 EJS files overriding the `_shared` base. Uses `@heroui/react` (Navbar, Card, Input, Chip, Button, Divider, ScrollShadow), `lucide-react` (tree-shaken, ~5KB per 50 icons), and Inter + Outfit Google Fonts. Tailwind config injects the shop's brand color into HeroUI's theme via `<%= primaryColor %>`. App.jsx reduced from ~928 lines (classic inline) to ~500 lines (HeroUI-powered) by delegating navbar, card, input, chip, and button styling to HeroUI.
...
- Dark mode: semantic tokens only — raw palette classes fail `scripts/check-colors.mjs`; intentional art is allowlisted or in excluded files
